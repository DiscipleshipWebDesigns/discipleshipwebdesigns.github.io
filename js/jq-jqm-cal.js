(function($) {
   $.jqmCalendar = function(element, options) {
      ...
      function init() {
         ...
         $listview = $("<ul data-role='listview' data-inset=\"true\" data-split-icon=\"gear\"/>").insertAfter($table);
      ...
      function refresh(date) {
         ...
         for (    var   weekIndex = 0,
            ...
            var daysInWeekCount = 0,
               row = $("<tr style=\"height:65px\"/>").appendTo($tbody);
      ...
      $element.bind('change', function(event, begin) {
         ...
         for ( var   i = 0, event; event = plugin.settings.events[i]; i++ ) {
            if ( event[plugin.settings.end] > begin && event[plugin.settings.begin] < end ) {
               ...
               var description    = event[plugin.settings.description],
                   summary = event[plugin.settings.summary],
                   ...
                   url = event[plugin.settings.url],
                   icon = event[plugin.settings.icon],
                   id = event[plugin.settings.id],
                   timeString = beginTime + "-" + endTime;
               $("<li ><a href=\"#purchase?id="+id+"\" data-rel=\"popup\" data-position-to=\"window\" data-transition=\"pop\"><img src="+icon+">"+ ( ( timeString != "00:00-00:00" ) ? timeString : "" )+" "+summary  + "<br/>  " + description + "</a><a href="+url+" ></a></li>").appendTo($listview);   
            }
         }
